...
<div id="product-associations" class="panel product-tab">
	...
	<div class="form-group">
		<label class="control-label col-lg-3" for="product_autocomplete_input2">
			<span class="label-tooltip" data-toggle="tooltip"
				  title="{l s='You can indicate existing products as accessories for this product.'}{l s='Start by typing the first letters of the product\'s name, then select the product from the drop-down list.'}{l s='Do not forget to save the product afterwards!'}">
                {l s='Цвета'}
			</span>
		...
		<div class="col-lg-5">
			<input type="hidden" name="inputAccessories2" id="inputAccessories2" value="{foreach from=$accessories2 item=accessory}{$accessory.id_product}-{/foreach}" />
			<input type="hidden" name="nameAccessories2" id="nameAccessories2" value="{foreach from=$accessories2 item=accessory}{$accessory.name|escape:'html':'UTF-8'}¤{/foreach}" />
			<div id="ajax_choose_product">
				<div class="input-group">
					<input type="text" id="product_autocomplete_input2" name="product_autocomplete_input2" />
					<span class="input-group-addon"><i class="icon-search"></i></span>
				</div>
			</div>

			<div id="divAccessories2">
                {foreach from=$accessories2 item=accessory}
					<div class="form-control-static">
						<button type="button" class="btn btn-default delAccessory2" name="{$accessory.id_product}">
							<i class="icon-remove text-danger"></i>
						</button>
                        {$accessory.name|escape:'html':'UTF-8'}{if !empty($accessory.reference)}&nbsp;{l s='(ref: %s)' sprintf=$accessory.reference}{/if}
					</div>
