...
class AdminProductsControllerCore extends AdminController
{
    ...
    public function processAdd()
    {
        ...
        if ($this->object->add()) {
            ...
            $this->updateAccessories2($this->object);
                ...
                if ($object->update()) {
                    ...
                    if (in_array($this->context->shop->getContext(), array(Shop::CONTEXT_SHOP, Shop::CONTEXT_ALL))) {
                        ...
                        if ($this->isTabSubmitted('Associations')) {
                            ...
                            $this->updateAccessories2($object);
                        }
    ...
    public function updateAccessories2($product)
    {
        $product->deleteAccessories2();
        if ($accessories2 = Tools::getValue('inputAccessories2')) {
            $accessories_id2 = array_unique(explode('-', $accessories2));
            if (count($accessories_id2)) {
                array_pop($accessories_id2);
                $product->changeAccessories2($accessories_id2);
            }
        }
    }
    ...
    */
    ...
    public function initFormAssociations($obj)
    {
        ...
        // Accessories block
        $accessories2 = Product::getAccessoriesLight2($this->context->language->id, $product->id);

        if ($post_accessories2 = Tools::getValue('inputAccessories2')) {
            $post_accessories_tab = explode('-', $post_accessories2);
            foreach ($post_accessories_tab as $accessory_id) {
                if (!$this->haveThisAccessory($accessory_id, $accessories2) && $accessory = Product::getAccessoryById($accessory_id)) {
        ...
        $data->assign('accessories2', $accessories2);
