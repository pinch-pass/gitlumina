...
product_tabs['Associations'] = new function(){
    ...
    this.initAccessoriesAutocomplete2 = function (){
        $('#product_autocomplete_input2')
            ...
            }).result(self.addAccessory2);

        $('#product_autocomplete_input2').setOptions({
            extraParams: {
                excludeIds : self.getAccessoriesIds2()
            }
        });
    };
    ...
    this.getAccessoriesIds2 = function()
    {
        if ($('#inputAccessories2').val() === undefined)
            return id_product;
        return id_product + ',' + $('#inputAccessories2').val().replace(/\-/g,',');
    }
    ...
    this.addAccessory2 = function(event, data, formatted)
    {
        if (data == null)
            return false;
        var productId = data[1];
        ...
        var $divAccessories2 = $('#divAccessories2');
        var $inputAccessories2 = $('#inputAccessories2');
        var $nameAccessories2 = $('#nameAccessories2');

        /* delete product from select + add product line to the div, input_name, input_ids elements */
        $divAccessories2.html($divAccessories2.html() + '<div class="form-control-static"><button type="button" class="delAccessory2 btn btn-default" name="' + productId + '"><i class="icon-remove text-danger"></i></button>&nbsp;'+ productName +'</div>');
        $nameAccessories2.val($nameAccessories2.val() + productName + '¤');
        $inputAccessories2.val($inputAccessories2.val() + productId + '-');
        $('#product_autocomplete_input2').val('');
        $('#product_autocomplete_input2').setOptions({
            extraParams: {excludeIds : self.getAccessoriesIds2()}
        });
    };
    ...
    this.delAccessory2 = function(id)
    {
        var div = getE('divAccessories2');
        var input = getE('inputAccessories2');
        var name = getE('nameAccessories2');
        ...
        for (i in inputCut)
        {
            ...
            if (inputCut[i] != id)
            {
                input.value += inputCut[i] + '-';
                name.value += nameCut[i] + '¤';
                div.innerHTML += '<div class="form-control-static"><button type="button" class="delAccessory2 btn btn-default" name="' + inputCut[i] +'"><i class="icon-remove text-danger"></i></button>&nbsp;' + nameCut[i] + '</div>';
            }
            else
                $('#selectAccessories').append('<option selected="selected" value="' + inputCut[i] + '-' + nameCut[i] + '">' + inputCut[i] + ' - ' + nameCut[i] + '</option>');
        }
	...
	this.onReady = function(){
        ...
        self.initAccessoriesAutocomplete2();
		self.getManufacturers();
		$('#divAccessories').delegate('.delAccessory', 'click', function(){
			self.delAccessory($(this).attr('name'));
		});
        $('#divAccessories2').delegate('.delAccessory2', 'click', function(){
            self.delAccessory2($(this).attr('name'));
        });
		if (display_multishop_checkboxes)
